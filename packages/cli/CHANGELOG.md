...
- [#126](https://github.com/jtmthf/scholia/pull/126) [`4b6d519`](https://github.com/jtmthf/scholia/commit/4b6d5199a474e0f2efbd62173c489ab87fe09154) Thanks [@jtmthf](https://github.com/jtmthf)! - Align the comment layer's UI copy with CONTEXT's vocabulary (Comment, Page, Chat), fix the submit button always reading "Comment" (even when starting a private Chat or replying), and show who deleted a Comment and when on its tombstone.
...
- [#129](https://github.com/jtmthf/scholia/pull/129) [`fe1c131`](https://github.com/jtmthf/scholia/commit/fe1c131a39acdab34cd349ec061b348ee3c9c819) Thanks [@jtmthf](https://github.com/jtmthf)! - Fix the no-DB test script so it runs without Postgres. Vitest is now configured with `no-db` and `db` projects; `pnpm test:no-db` runs only the `no-db` project, which excludes the per-run database global setup, restoring the CI check on runners that have no database.

- [#130](https://github.com/jtmthf/scholia/pull/130) [`49b690f`](https://github.com/jtmthf/scholia/commit/49b690fef249129f9082ebd0eff887bc732ed3ab) Thanks [@jtmthf](https://github.com/jtmthf)! - Fix the comment rail clipping Conversations past a viewport's worth in Local Preview — the rail's sticky box is now its own scroll container, so every Conversation stays reachable.
