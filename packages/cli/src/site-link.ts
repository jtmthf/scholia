...
// A `.scholia` marker links a local target to the hosted Site it was shared as, so
// re-running `scholia share` in the same place uploads a new Version rather than
...
// ~/.scholia/credentials — so it's safe to commit alongside the docs.
const MARKER = ".scholia";
...
// Read the `.scholia` marker in `dir`, or null when absent/unparseable.
...
// Write (or overwrite) the `.scholia` marker in `dir`.
