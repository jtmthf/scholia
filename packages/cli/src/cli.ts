...
// Hosted commands (share/chats/state/rotate-*/delete-site) talk to @collab/server,
// which is out of scope for the v0.1 Local Preview release. They stay in the source
// but are only registered when COLLAB_HOSTED=1, so `--help` and the binary surface
...
const HOSTED_ENABLED = process.env.COLLAB_HOSTED === "1";
...
// Registers every server-backed command. Gated behind COLLAB_HOSTED so the default
